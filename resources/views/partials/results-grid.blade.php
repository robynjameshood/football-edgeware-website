...
@php
    $resultGroups = [
        [
            'key' => '1H',
            'title' => 'First Half List',
            'results' => array_values(array_filter($records, static fn ($record) => is_array($record) && ($record['currentHalf'] ?? null) === '1H')),
        ],
        [
            'key' => '2H',
            'title' => 'Second Half List',
            'results' => array_values(array_filter($records, static fn ($record) => is_array($record) && ($record['currentHalf'] ?? null) === '2H')),
        ],
    ];
@endphp
...
@else
    <div class="results-sections">
        @foreach ($resultGroups as $group)
            <details class="results-group" open>
                <summary class="results-group-summary">
                    <span>{{ $group['title'] }}</span>
                    <span class="results-group-count">{{ count($group['results']) }} fixture{{ count($group['results']) === 1 ? '' : 's' }}</span>
                </summary>

                @if (empty($group['results']))
                    <div class="notice results-group-empty">
                        No {{ strtolower($group['title']) }} fixtures are available for this date.
                    </div>
                @else
